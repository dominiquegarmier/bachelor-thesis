...
This will be familiar to anyone who has seen \emph{persistent homology} where such properties are referred to as \emph{persistence} \cite[Chap.~3]{Carlsson2014}. As an example of a dendogram consider figure \ref{fig:dendogram_example}.
\begin{figure}[h]
\centering
\begin{tikzpicture}
    \node (a) at (0,-0.25) {a};
...
\end{tikzpicture}
\caption{A dendogram $\theta: \R_{\geq0} \to \P(X)$ with the four points $X = \{a,b,c,d\}$.}
\label{fig:dendogram_example}
\end{figure}

Notice how in this case the monotonicity condition is satisfied \eg\ $\theta(1) \refines \theta(2)$ as shown on the righthand side. The second condition of a dendogram is also met as it becomes trivial at the top and discrete at the bottom. Finally, the third condition tells us what value the dendogram takes whenever a ``merge happens''. As for the relevance of the last condition, consider the following remark.

\begin{myremark}{}{left_regularity_of_dendograms}
\todo[unify all these remarks about regularity]
Concerning the third condition of dendograms, since all our metric spaces are finite, we could decide to omit it in the definition of a dendogram. Dendograms would still have discrete scale \ie\ the dendogram would still be constant on intervals.
However, it would not be clear what value the dendogram would take at the endpoints of these intervals\footnote{There are other ways we could decide to fix this issue. \Eg\ we could consider equivalence relations of dendograms.}.
