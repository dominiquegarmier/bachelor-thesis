...
The map that assigns a based topological space $(X,x_0)$ to its fundamental group $\pi_1(X,x_0)$ and assigns continuous functions $f: (X,x_0) \to (Y,y_0)$ to the group homomorphism 
$$
...
Let $G_1, G_2$ be groups, $\phi: G_1 \to G_2$ be a group homomorphism and $\mathcal{G}_1,\mathcal{G}_2$ the corresponding categories from \ref{exa:group-category}. The goal is to construct a corresponding functor 
$$
...
\begin{equation*}
    \begin{tikzcd}
      ...
      \end{tikzcd}
\end{equation*}
commutes. This means that $\Ff$ preserves the underlying map of morphisms.
\end{notation}
