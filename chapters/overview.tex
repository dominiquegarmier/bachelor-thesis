...
In the simplest case this ``grouping'' is \emph{partition} of the original data points, this is also called \emph{classical clustering} (see Figure \ref{fig:intro_partition} in the Introduction). Alternatively we could assign a so-called \emph{dendrogram}
to the data points, which is referred to as \emph{hierarchical clustering}.  In this context a dendrogram is a (rooted) binary tree where the leaves are the data points (Figure \ref{fig:intro_dendrogram}). 
...
Let us now consider a clustering algorithm that will produce a dendrogram as an output. We will achieve this by consecutively merging clusters based on some distance criterion \cite[Sec.~4.2.2]{Everitt2011}.
...
A desirable property of a clustering algorithm might be that it remains invariant under certain transformations to the data.
