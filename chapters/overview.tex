...
\end{equation*}
is minimal, where $\mu_k := \frac{1}{|C_k|}\sum_{x \in C_k} x$\footnote{In case $C_k = \emptyset$ we set $\mu_k = 0 \in \R^d$.} is the \emph{center} of $C_k$.
\end{definition}
...
Let $d$ be a \emph{distance function}\footnote{In this case, a \emph{distance function} is just a function taking values in $\R_{\geq0}$.} between clusters. We start with the partition $B^{(0)}_j := \{x_j\}$ for $j = 1, \dots, n$.
Given a partition
$$
