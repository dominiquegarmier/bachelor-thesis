...
We first give a few examples of clustering algorithms, which are commonly studied in machine learning and data science. In particular, we discuss classical and hierarchical clustering algorithms and contextualize them with a few example computations and will see how they can fall short in certain cases.
For this, we base ourselves on the books \cite{Everitt2011} and \cite{Scitovski2021}.
...
In this context a \emph{dendrogram} is a (rooted) binary tree where the leaves are the data points. We will later define a more general notion of a dendrogram in section \ref{section__partitions}.

\begin{definition}{Linkage Clustering }{linkage_clustering}
Let $\{x_1, \dots, x_n\} \subset \R^n$ and $\mathfrak{d}$ be a \emph{distance function}\footnote{In this case, a \emph{distance function} between clusters is a function $\mathscr{P}(X) \times \mathscr{P}(X) \to \R_{\geq0}$.} between clusters. We start with the partition
$$
B^{(0)}_j := \{x_j\}$$
...
\end{definition}
Depending on the distance function $\mathfrak{d}$ we use we call \emph{linkage clustering} differently, \eg:

\begin{itemize}
    \item \emph{single-linkage clustering}: $\mathfrak{d}_\mathrm{min} (B_i, B_j) := \min\{d(x,y): x \in B_i, y \in B_j\}$,
    \item \emph{complete-linkage clustering}: $\mathfrak{d}_\mathrm{max} (B_i, B_j) := \max\{d(x,y): x \in B_i, y \in B_j\}$,
    \item \emph{average-linkage clustering}: $\mathfrak{d}_\mathrm{avg}(B_i,B_j) := \frac{1}{|B_i||B_j|} \sum_{x \in B_i, y \in B_j} d(x,y)$.
\end{itemize}
