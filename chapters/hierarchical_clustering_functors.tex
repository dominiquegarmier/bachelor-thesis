...
We define the \emph{shift} functor $\sigma_\lambda\colon \H \to \H$ for some $\lambda > 0$. Given $(X, \theta) \in \ob(\H)$ we set
$$
\sigma_\lambda(X, \theta(r)) := (X, \theta(r \cdot \lambda^{-1})) \quad \forall r \in \R_{\geq0}.
$$
...
\begin{definition}{}{}
A hierarchical clustering functor $\Hf\colon \M \to \H$ for $\M \in \{\iso,\inj,\gen\}$ is called \emph{scaling} if for all ${(X,d) \in \ob(\M)}$ and $\lambda > 0$ we have
$$
\Hf(X, \lambda \cdot d) = \sigma_\lambda \Hf(X,d)
$$
In other words for all $r \in \R_{\geq0}$ we have
$$
...
It is clear that $\Hf_\Cf$ is scaling since for $\lambda,r > 0$ and $(X,d) \in \ob(\M)$ we have
$$
\sigma_\lambda \Hf_\Cf(X,d;r) = \Hf_\Cf(X,d;r \lambda^{-1}) = \Cf(X, r^{-1} \lambda \cdot d) = \Hf_\Cf(X, \lambda \cdot d; r).
$$ 
...
\begin{proof}
Let $\Hf\colon \M \to \H$ be a hierarchical clustering functor fulfilling the modified \textsc{Kleinberg} conditions. \Ie\ In particular, we have $\M = \gen$.

By proposition \ref{prop:scaling_extension_correspondence}, there exists a regular spanning classical clustering functor $\Cf\colon \gen \to \C$ such that $\Hf = \Hf_\Cf$.

Since $\Hf$ is surjective we have that $\Cf$ is also surjective. So by Theorem \ref{thm:uniqueness_of_surjective_regular_functors}, we have that $\Cf = \Rf_{\lambda}$ for some $\lambda > 0$. Using $\delta := 1/\lambda$ and applying all the definitions we get that
$$
\Hf = \Hf_{\Rf_\lambda} = \sigma_\delta \Hf_{\Rf_1} = \sigma_\delta \Rf.
$$
\end{proof}
