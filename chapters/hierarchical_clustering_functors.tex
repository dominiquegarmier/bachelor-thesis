...
Similar to classical clustering functors, we also want to define some properties for hierarchical clustering functors. Moreover, from now on we will again assume that $\M \in \{\gen, \inj\}$ unless otherwise stated.
...
\Rf = \Hf_{\Rf_1}.
$$
Moreover, for $\lambda > 0$ and the shift functor $\sigma_\lambda$ we get that
$$
\sigma_\lambda \Rf = \sigma_\lambda \Hf_{\Rf_1} = \Hf_{\Rf_{1/\lambda}}.
$$
\end{example}
